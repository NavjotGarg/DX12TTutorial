...
ID3D12PipelineState* pipelineStateObject; // pso containing pipeline state object

ID3D12RootSignature* rootSignature; // root signature defines data shaders will access

D3D12_VIEWPORT viewport; // area that output from rasterizer will be stretched to

D3D12_RECT scissorRect; //  the ara to draw in. Pixels outside that area will be drawn to

ID3D12Resource* vertexBuffer; // default buffer in GPU memory that we will load vertex data for our triangle into

D3D12_VERTEX_BUFFER_VIEW vertexBufferView;	// structure containing a pointer to vertex data in gpu memory 
